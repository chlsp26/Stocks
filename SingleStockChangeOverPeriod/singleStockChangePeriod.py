...
def calculate_PL_percentage(data):
    ...
    data['Datetime'] = data['Date'].apply(strftime_with_offset)
    for i in data.index:
        if i == 0:
            percentages = percentages.append({'dateTime': data['Datetime'][i], 'percentage': round(
                ((data['Close'][i] - data['Open'][i])/data['Open'][i])*100, 2)}, ignore_index=True)
        else:
            percentages = percentages.append({'dateTime': data['Datetime'][i], 'percentage': round(
                ((data['Close'][i] - data['Close'][i-1])/data['Close'][i-1])*100, 2)}, ignore_index=True)
    return percentages
...
for i in stockmarketdates.index:
    try:
        ...
        data = stock.history(
            interval=d.interval, start=stockmarketdates[i], end=stockmarketdates[i+1])
        ...
        if len(all_stock_data) == 0:
            ...
            all_stock_data.rename(
                columns={'percentage': stockmarketdates[i]}, inplace=True)
        else:
            all_stock_data = pd.merge(all_stock_data, calculate_PL_percentage(
                data), how='outer', on='dateTime')
            all_stock_data.rename(
                columns={'percentage': stockmarketdates[i]}, inplace=True)
...
for i in range(len(all_stock_data.Date)-1, -1, -1):
    if i == 0:
        ...
        Change[all_stock_data.Date[i]] = round(
            ((all_stock_data['Open'][i] - all_stock_data['LTP'][i-1])/all_stock_data['LTP'][i-1])*100, 2)
all_stock_data.set_index("Date", inplace=True)
